                ...
                while (!stopAllThreads)
                {
                    try
                    {
                        ...
                        if (key != props["thread"])
                        {
                            ...
                            Threads.TraceConsole($"Error in key, expected:{key}, actual:{props["thread"]}, value is a valid but from an other query: {valueIsValid}");
                        }
                        if (checkContent && expectedSize != props["Value"].Length)
                        {
                            ...
                            Threads.TraceConsole($"Error in value lenght in thread '{key}', expected:{expectedSize}, actual:{props["Value"].Length}, value is a valid but from an other query: {valueIsValid}");
                        }
